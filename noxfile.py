...
def tests(session):
    ...
    try:
        ...
        session.run(
            ...
            "--omit=src/saltext/consul/*",
            "--include=tests/*",
        )
        try:
            session.run("coverage", "report", "--show-missing", "--include=src/saltext/consul/*")
            ...
            #    "--include=src/saltext/consul/*,tests/*"
            # )
        finally:
...
def gen_api_docs(session):
    ...
    session.run(
        ...
        "docs/ref/",
        "src/saltext",
        "src/saltext/consul/config/schemas",
    )
